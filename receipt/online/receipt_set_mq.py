...
from sqlalchemy import create_engine, func, BigInteger, or_, and_
...
M_conn = MongoClient(
    host='mongodb://er_user:vE0UmSo1fV3q@dds-uf6a4f0597f9e3041.mongords.aliyuncs.com:3717,dds-uf6a4f0597f9e3042.mongords.aliyuncs.com:3717/er?replicaSet=mgset-13011481')
db_C = M_conn.er
collection = db_C.orderResult
...
def up_data_receipt(my_data):  # 将订单更新到数据库
    ...
    for i in my_data:
        ...
        if a.first():
            ...
            if r:
                if int(r.Content) != int(i['Count']):
                    b.Content = i['Count']
                    m.append({'res': '与上次设置不一致的设备', 'imei': b.IMEI})
                    try:
                        session.add(b)
                        session.commit()
                        session.close()
                ...
                b.Content = i['Count']
                m.append({'res': '从未设置过的设备', 'imei': b.IMEI})
                try:
                    session.add(b)
                    session.commit()
                    session.close()
...
if __name__ == "__main__":
    while 1:
        ...
        a = get_set_data()
        print(a)
        print('----------正在获取没有绑定订单的设备------------', datetime.now())
        b = get_no_order_imei()
        print(b)
