...
def getAuthImage(redis_c=redis_conn, uid=uid):
    ...
    if img_list:
        ...
        return {'code': False, 'res': '{} 不存在'.format(imgpath)}
    image = Image.open(imgscr).convert("RGBA")
    width = image.size[0]
    height = image.size[1]
    ...
    w = x + 40
    h = y + 40
    mask_b = Image.open(os.path.join(os.path.dirname(__file__), 'mask_b.png')).convert("RGBA")  # 大图蒙板
    mask_s = Image.open(os.path.join(os.path.dirname(__file__), 'mask_1x.png')).convert("RGBA")  # 小图蒙板
    coordinate = (x, y, w, h)
    # draw = ImageDraw.Draw(image)
    # draw.rectangle((x, y, w+1, h+1), outline="black", width=1)
    region = image.crop(coordinate)  # region 小图，image 原图
    m2 = Image.new('RGBA', region.size)
    m2.paste(region, mask=mask_s)

    region = ImageEnhance.Contrast(m2).enhance(2.0)  # 对比度增强
    region = ImageEnhance.Brightness(region).enhance(1.0)  # 亮度增强
    ...
    image2 = Image.open(imgscr).convert("RGBA")
    mask_b = ImageEnhance.Contrast(mask_b).enhance(20.0)  # 对比度增强
    mask_b = ImageEnhance.Brightness(mask_b).enhance(1.5)  # 亮度增强
    image2.paste(mask_b, (x, y), mask=mask_b)
    buffered2 = BytesIO()
