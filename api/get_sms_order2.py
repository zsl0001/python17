...
def get_no_start_msg_reason(index_id,
    ...
    if res:
        ...
        return "{'res': '短信已发出/订单已到达!', 'code': 10001}", 10001
...
def process_control(**kwargs):  # 流程控制入口
    ...
    # loc_type = judge_location_type(index_id) SmsSendtrack.Index_RootOrderID
    res = db.session.query(SmsSendtrack).filter(SmsSendtrack.Index_RootOrderID == index_id,
    ...
    try:
        ...
        print(res.Index_NoStartMsgRes,process_control,index_id)
        print(res.Index_NoArriveMsgRes,process_control,index_id)
        print(res.Index_NoArriveRes,process_control,index_id)
        print(datetime.now())
        db.session.commit()
    except Exception as e:
        print(e, 10086)
